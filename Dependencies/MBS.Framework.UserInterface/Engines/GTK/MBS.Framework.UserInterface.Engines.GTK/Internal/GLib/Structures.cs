	...
	internal static class Structures
	{
		...
		public struct Value : IDisposable
		{
			...
			public static explicit operator Enum(Value val)
			{
				...
				{
					return (Enum)Enum.ToObject(GType.LookupType(val.type), GObject.Methods.g_value_get_flags(ref val));
				}
				return (Enum)Enum.ToObject(GType.LookupType(val.type), GObject.Methods.g_value_get_enum(ref val));
			}
			...
			public object Val
			{
				get
				{
					...
					if (type == GType.UChar.Val)
					{
						return (byte)this;
					}
					if (type == GType.Char.Val)
					{
						return (sbyte)this;
					}
					if (type == GType.Int.Val)
					{
						return (int)this;
					}
					if (type == GType.UInt.Val)
					{
						return (uint)this;
					}
					if (type == GType.Int64.Val)
					{
						return (long)this;
					}
					if (type == GType.UInt64.Val)
					{
						return (ulong)this;
					}
					if (GObject.Methods.g_type_is_a(type, GType.Enum.Val) || GObject.Methods.g_type_is_a(type, GType.Flags.Val))
					{
						return (Enum)this;
					}
					if (type == GType.Float.Val)
					{
						return (float)this;
					}
					if (type == GType.Double.Val)
					{
						return (double)this;
					}
					if (type == GType.String.Val)
					{
						return (string)this;
					}
					if (type == GType.Pointer.Val)
					{
						return (IntPtr)this;
					}
					if (type == GType.Param.Val)
					{
						return GObject.Methods.g_value_get_param(ref this);
					}
					if (GObject.Methods.g_type_is_a(type, GType.Object.Val))
					{
						return (Object)this;
					}
					if (GObject.Methods.g_type_is_a(type, GType.Boxed.Val))
					{
						return ToBoxed();
					}
					if (type == IntPtr.Zero)
				...
				set
				{
					...
					else if (GObject.Methods.g_type_is_a(type, GType.Enum.Val))
					{
						GObject.Methods.g_value_set_enum(ref this, (int)value);
					}
					else if (GObject.Methods.g_type_is_a(type, GType.Flags.Val))
					{
			...
			*/
			...
			private object ToBoxed()
			{
				...
				Type type = GType.LookupType(this.type);
				if (type == (Type)null)
				{
					throw new Exception("Unknown type " + new GType(this.type).ToString());
				}
				/*
			...
			{
				if (GObject.Methods.g_type_is_a(type, GType.Boxed.Val) /* && !(val is IWrapper)*/)
				{
