...
namespace UniversalEditor.Plugins.Multimedia.DataFormats.Audio.Synthesized.MusicXML
{
															...
															if (tag_elTagScorePartChild.Attributes["rel"] != null)
															{
																...
																if (text != null)
																{
																	if (text == "synthesizer")
																	{
																		if (FileName != null)
																		{
																			track.Synthesizer = Reflection.GetAvailableObjectModel<VoicebankObjectModel>(Path.MakeAbsolutePath(tag_elTagScorePartChild.Attributes["href"].Value, System.IO.Path.GetDirectoryName(FileName)));
																		}
																	}
																}
															}
