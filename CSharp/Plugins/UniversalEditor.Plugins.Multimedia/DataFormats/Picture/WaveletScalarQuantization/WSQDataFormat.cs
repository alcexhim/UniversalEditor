...
namespace UniversalEditor.Plugins.Multimedia.DataFormats.Picture.WaveletScalarQuantization
{
	public class WSQDataFormat : DataFormat
	{
		...
		protected override void LoadInternal(ref ObjectModel objectModel)
		{
			...
			if (signature != Internal.Constants.SOI_WSQ) throw new DataFormatException(Localization.StringTable.ErrorDataFormatInvalid);
			...
			for (int i = 0; i < height; i++)
			{
				...
				{
					pic.SetPixel(System.Drawing.Color.FromArgb(cdata[pix], cdata[pix], cdata[pix]), j, i);
					pix++;
				}
			}
		}
