...
namespace UniversalEditor.Plugins.Multimedia.DataFormats.Playlist
{
	public class SMILDataFormat : XMLDataFormat
	{
		public override DataFormatReference MakeReference()
		{
			...
			dfr.Clear();
			dfr.Filters.Add("SMIL playlist", new byte?[][] { new byte?[] { new byte?(60), new byte?(63), new byte?(119), new byte?(112), new byte?(108) }, new byte?[] { new byte?(60), new byte?(63), new byte?(122), new byte?(112), new byte?(108) }, new byte?[] { new byte?(60), new byte?(63), new byte?(120), new byte?(109), new byte?(108) } }, new string[] { "*.wpl", "*.zpl", "*.smil" });
