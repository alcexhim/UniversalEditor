...
namespace UniversalEditor.Compression.Puyo
{
	...
	{
		public override string Name { get { return "LZ00"; } }
		...
		public override void Decompress(Stream inputStream, Stream outputStream, int inputLength, int outputLength)
		{
			...
			try
			{
				uint num = inputStream.ReadUInt(4L);
				uint num2 = inputStream.ReadUInt(48L);
				uint xValue = inputStream.ReadUInt(52L);
		...
		public override void Compress(Stream inputStream, Stream outputStream)
		{
			...
			try
			{
				...
				/*
				if (Path.GetExtension(mvarFileName).ToLower() == ".mrz")
				{
					filename = Path.GetFileNameWithoutExtension(mvarFileName) + ".mrg";
				}
				else
				{
		...
		public override string DecompressFilename(ref Stream inputStream, string filename)
		{
			string text = inputStream.ReadString(16L, 32, Encoding.GetEncoding("Shift_JIS"));
			string result;
		...
		public override string CompressFilename(ref Stream inputStream, string filename)
		{
			string extension;
			string result;
		...
		public override bool Check(ref Stream inputStream, string filename)
		{
			bool result;
			try
			{
				result = (inputStream.ReadString(0L, 4) == "LZ00");
			}
			catch
			{
				result = false;
