...
using UniversalEditor.Engines.WindowsForms.Dialogs;
...
namespace UniversalEditor.Engines.WindowsForms
{
	public partial class MainWindow : AwesomeControls.Window, IHostApplicationWindow
	{
		...
		private void RecursiveLoadCustomToolbar(Toolbar toolbar)
		{
			...
			cbt.Tag = toolbar;
			foreach (UniversalEditor.UserInterface.MenuItem mi in toolbar.Items)
			{
		...
		private void RecursiveLoadCustomToolbarMenuItem(UniversalEditor.UserInterface.MenuItem mi, AwesomeControls.CommandBars.CBToolBar cbt)
		{
		...
		public void RefreshCommand(object nativeCommandObject)
		{
			if (nativeCommandObject is ToolStripItem)
			{
				...
				UniversalEditor.UserInterface.MenuItem mi = (tsi.Tag as UniversalEditor.UserInterface.MenuItem);
		...
		private void RecursiveLoadCustomMenuItems(UniversalEditor.UserInterface.MenuItem item, ToolStripDropDownItem tsiParent = null)
		{
			if (item is ActionMenuItem)
			{
				...
				if (tsiParent != null && tsiParent.DropDownItems.ContainsKey(item.Name) && tsiParent.DropDownItems[item.Name] is ToolStripMenuItem)
				{
					...
					foreach (UniversalEditor.UserInterface.MenuItem mi in ami.Items)
					{
		...
		public void RefreshDocument()
		{
			...
			if (CurrentDocument != null)
			{
				...
				if (editor != null)
				{
					foreach (UniversalEditor.UserInterface.MenuItem mi in editor.MenuBar.Items)
					{
